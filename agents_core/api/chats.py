...
from .converters import branch_out, chat_out, message_out, settings_out
...
def list_chats(agent_id: Optional[str] = Query(None), repo: Repository = Depends(get_repository)) -> List[ChatOut]:
    ...
    return [chat_out(c, repo.chat_stats(c, branch=1), repo) for c in chats]
...
def get_chat(chat_id: str, branch: Optional[int] = Query(1), repo: Repository = Depends(get_repository)) -> ChatOut:
    ...
    return chat_out(chat, repo.chat_stats(chat, branch=branch), repo)
...
def get_chat_settings(chat_id: str, repo: Repository = Depends(get_repository)) -> SettingsOut:
    chat = repo.get_chat(chat_id)
    return settings_out(chat.settings, tools_sources=repo._tools_sources_for(chat.settings, chat))
...
def update_chat_settings(chat_id: str, patch: SettingsPatch, repo: Repository = Depends(get_repository)) -> SettingsOut:
    chat = repo.get_chat(chat_id)
    updated = repo.update_chat_settings(chat_id, patch.to_payload())
    return settings_out(updated, tools_sources=repo._tools_sources_for(updated, chat))
